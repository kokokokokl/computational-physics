...
class simple_harmonic():
    ...
    def plot(self):
        plt.plot(self.t,self.theta,'g-',label="Euler method")
class cromer(simple_harmonic):
    def calculate(self):
        for i in range(self.n):
            ...
            self.t.append(self.t[-1]+self.dt)
    def plot(self):
        plt.plot(self.t,self.theta,'r-',label="Euler-Cromer method")
a=simple_harmonic(0,10*math.pi/180,0.01,20,0)
a.calculate()
a.plot()
b=cromer(0,10*math.pi/180,0.01,20,0)
b.calculate()
b.plot()
plt.xlabel("time(s)")
